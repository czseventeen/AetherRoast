...
class TemperatureController:
    def __init__(
        ...
        # Load roast profile if provided
    ...
    def log_step(self, elapsed, stage, target_temp, actual_temp, on_time):
        ...
        self.csv_writer.writerow([time.time(), elapsed, mmss, stage, target_temp, actual_temp, on_time])
        self.log_fh.flush()

    # --------------------
    # Roast Stage
    # --------------------
    def get_stage(self, temp):
        if temp < 160:
            return "Drying"
        elif temp < 190:
            return "Maillard"
    ...
    def control_step(self):
        ...
        stage = self.get_stage(temp)
        mmss = time.strftime("%M:%S", time.gmtime(elapsed))

        # Console log
        print(f"Elapsed: {mmss} | Stage: {stage} | Temp: {temp:.2f}°C | Target: {self.setpoint:.2f}°C | SSR ON: {on_time:.2f}s")

        # Log to CSV
        self.log_step(elapsed, stage, self.setpoint, temp, on_time)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 roast_controller.py <roast_profile.csv>")
        sys.exit(1)
